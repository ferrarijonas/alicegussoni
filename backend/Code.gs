...
function doGet(e) {
  ...
  if (e && e.parameter && e.parameter.acao === 'adminarea') {
    ...
    return responder({ ok: true, url: 'https://ferrarijonas.github.io/alicegussoni/aluno.html?token=' + encodeURIComponent(adminToken(e.parameter.curso)) }, e.parameter.callback);
  }
...
function adminToken(curso) {
  var sel = curso === 'imersao' ? ':imersao' : curso === 'descoberta' ? ':descoberta' : '';
...
function areaAlunoAdmin(curso) {
  var chaves = curso === 'imersao' ? ['Imersão'] : curso === 'descoberta' ? ['Descoberta'] : ['Descoberta', 'Imersão'];
  var cursos = [];
...
function buscarAlunoDados(token) {
  var cursoAdmin = token === adminToken('descoberta') ? 'descoberta' : token === adminToken('imersao') ? 'imersao' : token === adminToken() ? 'ambos' : '';
